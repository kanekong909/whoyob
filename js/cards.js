...
// Multer sube directo a Cloudinary
...
router.put('/:id', upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    // Si subieron foto nueva, borrar la vieja de Cloudinary
    if (req.file) {
      if (old.photo_url) {
        const publicId = old.photo_url.split('/').slice(-2).join('/').replace(/\.[^.]+$/, '');
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (rows[0].photo_url) {
      const publicId = rows[0].photo_url.split('/').slice(-2).join('/').replace(/\.[^.]+$/, '');
      cloudinary.uploader.destroy(publicId).catch(() => {});
    }
